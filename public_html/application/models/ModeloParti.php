...

{
...

    {
        ...
        $prueba = '<img class="img-responsive" style="width: 130px; height:125px;" src="http://www.tarreoinformatica2019.com/lib/img/Juegos/';
...
//        $this->db->where("j.estadoJuego", 1);
...

    {
        ...
        $prueba = '<img class="img-responsive" style="width: 50px; height:50px;" src="http://www.tarreoinformatica2019.com/lib/img/Jugadores/';
...

    {
        ...
        $prueba = '<img class="img-responsive" style="width: 140px; height:100px;" src="http://www.tarreoinformatica2019.com/lib/img/Equipos/';
	...
	$this->db->where('estadoEquipo_Participante', 1);
	...
	$this->db->where('a.teamEquipo', $idEquipo);
    ...
    function verSolicitudesXParticipante($id){
        $this->db->select("e.idEquipo_Participante,a.nombreEquipo, concat(p.nombreParticipante,' ',p.apellidoParticipante) as nombreParticipante,k.nombreEstadoP");
        $this->db->from("equipo_participante e");
        $this->db->join("equipo a", "a.idEquipo = e.teamEquipo");
        $this->db->join("participante p", "p.idParticipante = a.capitanEquipo");
        $this->db->join("estado_participante k", "k.idEstadoP = e.estadoEquipo_Participante");
        $this->db->where("e.participanteEQuipo", $id);
        return $this->db->get();
    }

	function eliminarPostulacion($idJuego, $idParticipante)
    {
        $this->db->select('count (*)');
        $this->db->from('postulantes');
        $this->db->where('participantePostulante', $idParticipante);
        $this->db->where('juegoPostulante', $idJuego);
        $resultado = $this->db->count_all_results();
	...
	function EditarParticipante($idParticipante, $nombres, $apellidos, $correo, $numero, $nombre_imagen, $clave)
    { 
        $data = array(
...
$this->db->from('participante');
	  ...
	  function EditarParticipanteSinFoto($idParticipante, $nombres, $apellidos, $correo, $numero, $clave)
    {
        $data = array(
            "nombreParticipante" => $nombres,
            "apellidoParticipante" => $apellidos,
...
$this->db->from('participante');
	  ...
	  function addSugerencia($asunto,$descripcion,$idParticipante){
        $data = array(
            "asuntoSugerencia" => $asunto,
            "descripcionSugerencia" => $descripcion,
            "participante_id" => $idParticipante
        );
        $this->db->insert('sugerencia', $data);
        return 'ok';
    }
