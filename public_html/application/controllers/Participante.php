...

{
    ...
    {
        if (count($this->session->userdata("participante")) > 0) {
            $this->load->view("Participante/Solicitudes.php");
        } else {
            $this->load->view('Errormsg');
        }
    }
    ...
    {
        if (count($this->session->userdata("participante")) > 0) {
            $this->load->view("Participante/Perfil.php");
        } else {
            $this->load->view('Errormsg');
        }
    }
        ...
        if (count($this->session->userdata("participante")) > 0) {
            ...
            if ($nombre_imagen == null || $tipo_imagen == null || $tamano_imagen == null) {
                ...
                if ($tamano_imagen <= 10000000) {
                    ...
                    $carpeta_destino = $_SERVER['DOCUMENT_ROOT'] . '/lib/img/Equipos/';
...

    {
	 ...
	 public function verSolicitudesXParticipante()
    {
        if (count($this->session->userdata("participante")) > 0) {
            $user = $this->session->userdata("participante");
            $id = $user[0]->idParticipante;
            ...
            $start = intval($this->input->get("start"));
            $length = intval($this->input->get("length"));
            $books = $this->modeloParti->verSolicitudesXParticipante($id);
            $data = array();
            foreach ($books->result() as $r) {
                $data[] = array(
                    $r->idEquipo_Participante,
                    $r->nombreEquipo,
                    $r->nombreParticipante,
                    $r->nombreEstadoP,
                );
...
public function eliminarPostulacion()
    {
        if (count($this->session->userdata("participante")) > 0) {
            $user = $this->session->userdata("participante");
            $idParticipante = $user[0]->idParticipante;
            $idJuego = $this->input->post("id");
            $resultado = $this->modeloParti->eliminarPostulacion($idJuego, $idParticipante);
            if ($resultado == "ok") {
                echo json_encode(array("msg" => "ok"));
            } else if ($resultado == "vacio") {
                echo json_encode(array("msg" => "vacio"));
            }
        } else {
            $this->load->view('Errormsg');
        }
	...
	public function editarPerfil()
    {
        if (count($this->session->userdata("participante")) > 0) {
            ...
            if ($nombre_imagen == null || $tipo_imagen == null || $tamano_imagen == null) {
                ...
                $resultado = $this->modeloParti->EditarParticipanteSinFoto($idParticipante, $nombres, $apellidos, $correo, $numero, $clave);

                $this->session->set_userdata("participante", $resultado);

                redirect("Perfil");
            } else {
                if ($tamano_imagen <= 10000000) {
                    $carpeta_destino = $_SERVER['DOCUMENT_ROOT'] . '/lib/img/Jugadores/';
                    $nombre_imagen = $hora . $nombre_imagen;
                    ...
                    $resultado = $this->modeloParti->EditarParticipante($idParticipante, $nombres, $apellidos, $correo, $numero, $nombre_imagen, $clave);

move_uploaded_file($_FILES['foto']['tmp_name'], $carpeta_destino . $nombre_imagen);
                    $this->session->set_userdata("participante", $resultado);

                    redirect("Perfil");                } else {
                    echo "El tama�o de la imagen supera el limite";
                }
            }
        } else {
            $this->load->view('Errormsg');
	...
	public function addSugerencia(){
        if (count($this->session->userdata("participante")) > 0) {
            $user = $this->session->userdata("participante");
            $idParticipante = $user[0]->idParticipante;
            $asunto = $this->input->post("asunto");
            $descripcion = $this->input->post("descripcion");
            $resultado = $this->modeloParti->addSugerencia($asunto,$descripcion,$idParticipante);
            if ($resultado == "ok") {
                echo json_encode(array("msg" => "ok"));
            } 
        } else {
